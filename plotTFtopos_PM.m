...
figure, 
for s = 1:length(allsubj)+1
    subplot(length(allsubj)+1,3,spos(s)), hold on
    topodata.avg = alphatopo(:,s);
    ft_topoplotER(cfgtopo, topodata);
    
    subplot(length(allsubj)+1,3,spos(s)+1), hold on
    topodata.avg = betatopo(:,s);
    ft_topoplotER(cfgtopo, topodata);
    
    subplot(length(allsubj)+1,3,spos(s)+2), hold on
...
figure, 
for s = 1:length(allsubj)+1
    subplot(2,length(allsubj)+1,s), hold on
    ...
    subplot(2,length(allsubj)+1,s+4), hold on
...
figure, 
for s = 1:length(allsubj)+1
    subplot(4,length(allsubj)+1,s), hold on
    ...
    subplot(4,length(allsubj)+1,s+4), hold on
    ...
    subplot(4,length(allsubj)+1,s+8), hold on
    ...
    subplot(4,length(allsubj)+1,s+12), hold on
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   PRECEDENT TIMESTAMP POWER ANALYSIS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
file_name = '_samplewise_zsamp_output_LIonly.mat';

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%-- 11)  LPR PREDICTIVE LATERALIZATION - PRECEDENT TIMESTAMP POWER
%   (Analogous to 4)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Load and process data
TposteriorS_LI_all=[];
...
TposteriorS_LI_all(:,:,end+1) = nanmean(TposteriorS_LI_all,3);
...
figure, 
for s = 1:length(allsubj)+1
    subplot(1,length(allsubj)+1,s), hold on
    plot(samples,mean(TposteriorS_LI_all(freqs>=alphabnd(1) & freqs<=alphabnd(2),:,s),1),'Color',gcols(2,:))  % alpha = green
    plot(samples,mean(TposteriorS_LI_all(freqs>=betabnd(1) & freqs<=betabnd(2),:,s),1),'Color',bcols(2,:))  % beta = blue
    plot(samples,mean(TposteriorS_LI_all(freqs>=allbnd(1) & freqs<=allbnd(2),:,s),1),'Color',kcols(2,:))  % combined = black
    
    line([0 0],get(gca, 'ylim')','LineStyle','-','Color',[0 0 0],'LineWidth',1.25)
    line([samples; samples],repmat(get(gca, 'ylim')',1,length(samples)),'LineStyle','--','Color',[0.5 0.5 0.5])
    line([-0.1 samples(end)+smptimes(end)],[0 0],'LineStyle','-','Color',[0.3 0.3 0.3])
    xlim([-0.1 samples(end)+smptimes(end)])
    xlabel('# Sample'), ylabel('T-score')
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%-- 12)  LLR PREDICTIVE LATERALIZATION - PRECEDENT TIMESTAMP POWER
%   (Analogous to 5)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
TllrS_LI_all(:,:,end+1) = nanmean(TllrS_LI_all,3);
...
for s = 1:length(allsubj)+1
    subplot(1,length(allsubj)+1,s), hold on
    plot(samples,mean(TllrS_LI_all(freqs>=alphabnd(1) & freqs<=alphabnd(2),:,s),1),'Color',gcols(2,:))  % alpha = green
    plot(samples,mean(TllrS_LI_all(freqs>=betabnd(1) & freqs<=betabnd(2),:,s),1),'Color',bcols(2,:))  % beta = blue
    plot(samples,mean(TllrS_LI_all(freqs>=allbnd(1) & freqs<=allbnd(2),:,s),1),'Color',kcols(2,:))  % combined = black
...
% % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % %-- 13)  LLR*SURPRISE INTERACTION ON LATERALIZATION - PRECEDENT TIMESTAMP POWER
% % %   (Analogous to 6)
% % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % % Load and process data
% % TpriorWllrS_LI_all=[]; TpriorllrS_LI_all=[]; TllrXsurpriseS_LI_all=[];
...
% %     subplot(1,length(allsubj)+1,s), hold on
% %     for samp = 2:samples
% %         plot(samples(samp),mean(TllrXsurpriseS_LI_all(freqs>=alphabnd(1) & freqs<=alphabnd(2),samp-1,s),1),'Color',gcols(2,:))  % alpha = green
% %         plot(samples(samp),mean(TllrXsurpriseS_LI_all(freqs>=betabnd(1) & freqs<=betabnd(2),samp-1,s),1),'Color',bcols(2,:))  % beta = blue
% %         plot(samples(samp),mean(TllrXsurpriseS_LI_all(freqs>=allbnd(1) & freqs<=allbnd(2),samp-1,s),1),'Color',kcols(2,:))  % combined = black
% %     end
% %     line([0 0],get(gca, 'ylim')','LineStyle','-','Color',[0 0 0],'LineWidth',1.25)
% %     line([samples; samples],repmat(get(gca, 'ylim')',1,length(samples)),'LineStyle','--','Color',[0.5 0.5 0.5])
% %     line([-0.1 samples(end)+smptimes(end)],[0 0],'LineStyle','-','Color',[0.3 0.3 0.3])
