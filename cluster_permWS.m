...
%                    determines how clusters are defined
%          alpha   = alpha level applied to cluster test statistic
%
% Outputs: sig_ts  = vector of length time, with 1 marking significant
%                    clusters and nan elsewhere

function [sig_ts,sig_ts_uncorr,cP,permdist] = cluster_permWS(data,nperm,clustalpha,alpha)

% Isolate clusters in observed data and calculate associated test statistics
for t = 1:size(data,2)
    [Hobs(t),~,~,ts] = ttest(data(:,t,1),data(:,t,2),'alpha',clustalpha);
    Tobs(t) = ts.tstat;
end
...
if ~isempty(find(Hobs==1))  % making sure there's a significant effect at at least one time point in observed data
    [Cobs,nclust] = get_clusts(Hobs);
else
    error('No significant effects in observed data... :-/')
end
...
for p = 1:nperm
    ...
    Hperm=[]; Tperm=[]; cTperm=[];
    for t = 1:size(cdata,2)
        [Hperm(t),~,~,ts] = ttest(cdata(:,t,1),cdata(:,t,2),'alpha',clustalpha);
        Tperm(t) = ts.tstat;
    end
